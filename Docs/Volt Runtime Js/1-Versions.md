...
- `[x]` mantener `spa-lab` alineado con el wiring real de rutas demo (provider, indice visual `/spaReactive` y smoke tests del skeleton sincronizados para `/cacheExample`, `/fragmentCache`, `/runtimeState`, `/runtimePersist` y `/runtimeRequestLab`)
...
- `[x]` agregar `/runtimeMatrix` como runner manual para capturar snapshots coherentes de `telemetry`, `runtime asset`, `heap` (si existe) y exportar JSON por escenario
...
- `[x]` mejora real sobre peso de red de `/_volt/runtime.js`: `RuntimeAssetController` sirve el asset comprimido con `gzip` cuando el cliente lo acepta, bajando la transferencia observada de `354709 B` a `57513 B` en una carga fresca, sin sacar al `boot` del presupuesto (`25.8 ms` en la corrida post-fix)
- `[x]` mejora estructural real sobre `/_volt/runtime.js`: `vendor/voltstack/framework/tools/build-runtime.php` ahora minifica el bundle generado con `terser`, reduciendo el core de `354409 B` a `136301 B` y la transferencia gzip observada de `57513 B` a `35107 B`, manteniendo `boot` en presupuesto (~`30.3 ms`) y dejando `SkeletonSpaRoadmapTest.php` en verde (`54/54`)
